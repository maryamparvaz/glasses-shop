...
export default function catalog2() {
        ...
        const { perpage, updateUser, updateUser4, updateUser5, updateUser6, updateUser7 } = useStore();
        ...
        useEffect(() => {
                itemall.push(selectItem)
                localStorage.setItem('items', JSON.stringify(itemall));
                let localitems = localStorage.getItem("items")
                let db = []
                let flag = 0
                if (localitems != null) {
                        let getitem = JSON.parse(localitems);
                        getitem.map((val) => {
                                db.push(val)
                        })
                        localStorage.setItem('items', JSON.stringify(db));
                }
                const selectop = JSON.parse(localStorage.getItem('items'));
                // console.log();
                // updateUser7(selectop)
                // console.log(itemall);
        }, [selectItem])
        return (
                <>
